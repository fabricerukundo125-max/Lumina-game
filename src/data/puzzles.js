...
export const PUZZLES = [
  ...
  // ── Archetype: Restoration (P26) ────────────────────────────────────────────
  // Grid background transitions from dark ruin to warm green on solve.
  // Verified: R[3,0]→[3,2]\rot1→/→UP→[1,2]/fixed→R→[1,6]C ✓
  {
    id: 26,
    title: "The Ruined Garden",
    subtitle: "Something here is waiting to be remembered",
    hint: "Guide the light. Watch what wakes.",
    archetype: "restoration",
    sceneBefore: "radial-gradient(ellipse at 40% 60%, #0d1a0a 0%, #050a04 100%)",
    sceneAfter:  "radial-gradient(ellipse at 40% 60%, #1a4010 0%, #0a2006 50%, #142a08 100%)",
    grid: (() => {
      const g = b();
      g[1][2] = "/"; g[1][6] = "C#a8ff6b";
      g[3][2] = "\\";
      g[2][4] = "X"; g[4][4] = "X";
      return g;
    })(),
    sources:  [{ r: 3, c: 0, dir: "R", color: "#a8ff6b" }],
    crystals: [{ r: 1, c: 6, color: "#a8ff6b" }],
    movable:  ["3,2"],
  },

  // ── Archetype: Constellation (P27) ──────────────────────────────────────────
  // Sequence puzzle — beam passes through all 3 crystals bottom to top.
  // Golden lines draw between crystals as each is lit in order.
  // Verified: R[6,0]→[6,3]\rot1→/→UP col3→C[4,3]→C[3,3]→C[2,3] all lit ✓
  // Start: [6,3]\ deflects beam DOWN, exits. No crystals lit ✓
  {
    id: 27,
    title: "The Lost Triangle",
    subtitle: "Three stars. One ancient shape. Light them in order.",
    hint: "Point the mirror upward. The beam will find all three.",
    archetype: "constellation",
    sequence: ["4,3", "3,3", "2,3"],
    grid: (() => {
      const g = b();
      g[4][3] = "Cany";
      g[3][3] = "Cany";
    ...
    sources:  [{ r: 6, c: 0, dir: "R", color: "white" }],
    crystals: [
      { r: 4, c: 3, color: "any" },
      { r: 3, c: 3, color: "any" },
      { r: 2, c: 3, color: "any" },
  ...
  // ── Archetype: Color Harmony (P28) ──────────────────────────────────────────
  // Red fires DOWN col2. Blue fires DOWN col4.
  // Red movable / at [2,2]: default DOWN→LEFT (away). rot1→\ DOWN→RIGHT→M[2,3] ✓
  // Blue movable \ at [2,4]: default DOWN→RIGHT (away). rot1→/ DOWN→LEFT→M[2,3] ✓
  // Both at [2,3] = Magenta. M crystal lights ✓
  // Verified: start=0, solution rot1 on both movables ✓
  {
    id: 28,
    title: "The Merging Light",
    subtitle: "Two colors. One crystal. Find where they meet.",
    hint: "Red and blue must pass through the same point. Where they meet, magenta is born.",
    archetype: "color",
    grid: (() => {
      const g = b();
      g[2][3] = "M#cc44cc";
    ...
    sources: [
      { r: 0, c: 2, dir: "D", color: "#ff6b6b" },
      { r: 0, c: 4, dir: "D", color: "#6bcbff" },
    ],
